...
import { useDashboard } from '@/src/modules/dashboard'
...
export function DashboardContent() {
	const { stats, recentActivity, isLoading, error } = useDashboard()
	...
	const statsConfig = [
		{
			...
			icon: Users,
			trend: (stats?.patientChange ?? 0) >= 0 ? 'up' : 'down',
		},
		{
			title: 'Active Treatment Plans',
			value: stats?.activeTreatmentPlans ?? 0,
			change: stats?.planChange ?? 0,
			icon: FileText,
			trend: (stats?.planChange ?? 0) >= 0 ? 'up' : 'down',
		},
		{
			title: 'Safety Alerts',
